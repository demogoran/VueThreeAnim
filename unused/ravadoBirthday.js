...
var timeline=new TimelineMax({ repeat: 0, repeatDelay: 0.0, yoyo: false, onUpdate:()=>{
  ...
  //capturer.capture(document.querySelector('#three-container canvas'));
},
onComplete: ()=>{
  console.log("Done");
  capturer.stop();
...
function init() {
  ...
  var cSize=20;
  var animation = new ObjAnim(new THREE.PlaneGeometry(cSize*2.659279778393352, cSize, 100, 100), "in", new THREE.Color("white"));
...
function Slide(plane, animationPhase, uBackColor) {
  ...
  var material = new THREE.BAS.BasicAnimationMaterial({
    ...
    uniformValues: {
      map: new THREE.TextureLoader().load('models/bnw2.jpg'),
      canvasResolution: {
...
function ObjAnim(plane, animationPhase, uBackColor) {
  ...
  var material = new THREE.BAS.BasicAnimationMaterial({
    ...
    uniformValues: {
      map: texture,
      needSplash: 1,
      textureImage: new THREE.TextureLoader().load('models/ravadoBirthday.jpg'),
      //diffuse: new THREE.Color(0xFF00FF11)
    },
    vertexFunctions: [
      THREE.BAS.ShaderChunk['cubic_bezier'],
